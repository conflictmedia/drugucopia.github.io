...
export function searchSubstanceSummaries(
  ...
  for (const substance of summaries) {
    const fields: Array<{ value: string; field: string; exact: number; prefix: number; contains: number }> = [
      { value: substance.id, field: 'id', exact: 1, prefix: 0.95, contains: 0.85 },
      { value: substance.name, field: 'name', exact: 1, prefix: 0.9, contains: 0.8 },
      ...substance.commonNames.map((value) => ({ value, field: value, exact: 0.88, prefix: 0.78, contains: 0.65 })),
      ...substance.aliases.map((value) => ({ value, field: value, exact: 0.87, prefix: 0.7, contains: 0.6 })),
      { value: substance.class, field: 'class', exact: 0.55, prefix: 0.52, contains: 0.5 },
      { value: substance.description, field: 'description', exact: 0.32, prefix: 0.31, contains: 0.3 },
    ]
      ...
      const score = value === normalizedQuery
        ...
        : value.startsWith(normalizedQuery)
          ...
          : value.includes(normalizedQuery)
            ? candidate.contains
            : 0
      if (score > (best?.score ?? 0)) best = { substance, score, matchField: candidate.field }
    }
