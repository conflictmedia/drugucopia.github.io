...
import { loadSubstanceIndex, type SubstanceSummary } from '@/lib/substance-repository'
...
function customSummaries(): SubstanceSummary[] {
  try {
    const records = JSON.parse(localStorage.getItem(CUSTOM_SUBSTANCES_KEY) ?? '[]') as Substance[]
...
export function useSubstanceIndex(): SubstanceIndexState {
  const [state, setState] = useState<SubstanceIndexState>({ substances: [], loading: true, error: null })

  useEffect(() => {
    let active = true
    let builtIn: SubstanceSummary[] = []
    const publish = () => {
      if (active) setState({ substances: [...builtIn, ...customSummaries()], loading: false, error: null })
    }
    ...
    loadSubstanceIndex().then(
      ...
      (error: unknown) => active && setState({ substances: [], loading: false, error: error instanceof Error ? error : new Error(String(error)) }),
    )
    return () => {
      active = false
