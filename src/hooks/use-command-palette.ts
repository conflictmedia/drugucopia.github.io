...
import { searchSubstanceSummaries } from "@/lib/substance-repository";
import { useSubstanceIndex } from "@/hooks/use-substance-index";
import { generalGuides } from "@/lib/harm-reduction-guides";
...
export function useCommandPalette({
  ...
  const results = useMemo(() => {
    ...
    if (query) {
      const subs = searchSubstanceSummaries(substanceSummaries, query, 6);
      for (const r of subs) {
        const s = r.substance;
        out.push({
          id: `sub-${s.id}`,
  ...
  }, [query, doses, substanceSummaries, openDoseLogger]);
