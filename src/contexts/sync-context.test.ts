...
import { decryptData, deriveKey, encryptData, hashRoomName, mergeDoses } from './sync-context'
import type { DoseLog } from '@/types'

function dose(id: string, updatedAt: string, amount: number): DoseLog {
  return {
    id, updatedAt, createdAt: '2026-01-01T00:00:00.000Z', timestamp: '2026-01-01T00:00:00.000Z',
    substanceName: 'Fixture', categories: ['other'], amount, unit: 'mg', route: 'oral',
    duration: null, notes: null, mood: null, setting: null,
  }
}
...
describe('sync cryptography', () => {
  test('encrypts and decrypts structured payloads', async () => {
    ...
    const encrypted = await encryptData({ doses: [{ id: '1' }], version: 1 }, key)
    expect(encrypted.iv).not.toBe('')
    expect(encrypted.ciphertext).not.toContain('doses')
    expect(await decryptData(encrypted, key)).toEqual({ doses: [{ id: '1' }], version: 1 })
  })

  test('rejects decryption with a different password', async () => {
    const encrypted = await encryptData({ secret: true }, await deriveKey('one', 'room'))
    await expect(decryptData(encrypted, await deriveKey('two', 'room'))).rejects.toThrow()
  })

  test('room document hashes are deterministic and password-separated', async () => {
    expect(await hashRoomName('room', 'password')).toBe(await hashRoomName('room', 'password'))
    expect(await hashRoomName('room', 'password')).not.toBe(await hashRoomName('room', 'different'))
  })
})
...
describe('sync dose conflict resolution', () => {
  test('newer remote records win and simultaneous edits surface a conflict', () => {
    ...
    const baseline = new Map([['1', new Date('2026-01-02T00:00:00.000Z').getTime()]])
    const result = mergeDoses([local], [remote], new Set(), new Set(), baseline)
